...
<div class="card mb-4">
    ...
    <div class="card-body">
      <form method="post" action="/dashboard/posts/{{ $post->slug }}" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            <label for="formFile" class="form-label" >Post Cover</label></br>
            <input name="oldImage" type="hidden" value="{{$post->image}}">

            @if ($post->image)
              <img src="{{ '/storage/' . $post->image }}" class='img-preview img-fluid d-block col-md-5 rounded-3'>
            @else
              <img class='img-preview img-fluid d-block col-md-5 rounded-3'>  
            @endif
        ...
        <div class="mb-3">
          <label class="form-label" for="editor">Body</label>
          <textarea id="editor" class="form-control"  name="body">{{old('body', $post->body)}}</textarea>
          @error('body')
              <div class="invalid-feedback">
                {{ $message}}
              </div>
